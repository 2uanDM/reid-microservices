...
class STrack(BaseTrack):
    ...
    def result(self) -> List[float]:
        ...
        return (
            coords.tolist()
            + [self.track_id, self.score, self.cls, self.idx]
            + [self.state]
        )
...
class BYTETracker:
    """
    BYTETracker: A tracking algorithm built on top of YOLOv11 for object detection and tracking.
    ...
    def update(
        ...
        detections = self.init_track(
            dets, scores_keep, cls_keep, img if feats is None else feats
        )
        ...
        detections_second = self.init_track(
            dets_second, scores_second, cls_second, img if feats is None else feats
        )
        r_tracked_stracks = [
            strack_pool[i]
            for i in u_track
